...
def gen_adv_loss(logits, y, loss='logloss', mean=False):
    ...
    if loss == 'training':
        ...
        label_mask = y
        # pdb.set_trace()
        correct_logit = K.sum(label_mask * logits, axis=1)
        wrong_logit = K.max((1-label_mask) * logits - 1e4*label_mask, axis=1)
        out = -tf.nn.relu(correct_logit - wrong_logit + 50)
    else:
...
def gen_grad(x, logits, y, loss='cw'):
    """
