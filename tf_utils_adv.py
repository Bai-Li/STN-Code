...
def tf_test_acc(model, x, X_test, y_test, sd=0.0, num_iter=10):
    """
    Compute test error.
    """
    ...
    hist = np.zeros(shape=(len(y_test), 10))
    for i in range(num_iter):
        pred = batch_eval([x], [eval_prediction],
         [X_test + np.random.normal(scale=sd, size = X_test.shape)])[0]
        temp = np.zeros(shape=(len(y_test), 10))
        temp[np.arange(len(y_test)), pred.argmax(axis=1)] = 1
        hist += temp
    acc = np.sum(hist.argmax(axis=1)==y_test.argmax(axis=1))/100.
    return acc
